...
`gigamercado` extends the functionality of [`cyrcantile`](https://github.com/robert-werner/cyrcantile-opencl) with additional commands
...
|---|---|---|
| `burntiles.project_geom()` | Per-vertex `ct.xy()` loop | Vectorised `@njit` Mercator kernel |
| `burntiles.find_extrema()` | Python `zip`/`min`/`max` per feature | NumPy batched min/max on concatenated arrays |
| `burntiles.tile_extrema()` | `ct.tile()` calls | `@njit` `_tile_xy` scalar kernel |
...
```sh
cat data/ellada.geojson | gigamercado burn 10 | gigamercado union | fio collect
```
...
```
cat data/ellada.geojson | gigamercado burn 10 | gigamercado edges | gigamercado union | fio collect | geojsonio
```
...
Outputs a stream of unioned GeoJSON from an input stream of `[x, y, z]`s.
...
```sh
git clone git@github.com:robert-wermer/gigamercado.git
cd gigamercado
uv venv
